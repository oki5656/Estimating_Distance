...
class ESPNetv2Segmentation(nn.Module):
    ...
    def __init__(self, args, classes=21, dataset='pascal'):
        ...
        #                       BASE NETWORK
        # =============================================================
        self.base_net = EESPNet(args) #imagenet model #self.basenetはインスタンス
        del self.base_net.classifier  #del文：不要になった変数やオブジェクトなどのデータを消去
...
def espnetv2_seg(args):
    ...
    model = ESPNetv2Segmentation(args, classes=classes, dataset=dataset)#modelがESPNetv2Segmentationのインスタンス
    if weights:
        import os
        ...
        basenet_dict = model.base_net.state_dict()#重みの変数(dict)？
        model_dict = model.state_dict()#重みの変数(dict)？この辺の調査を行う
        overlap_dict = {k: v for k, v in pretrained_dict.items() if k in basenet_dict}#重みの変数(dict)？この辺の調査を行う
        #print("type(basenet_dict),type(model_dict),type(overlap_dict) = ",type(basenet_dict),type(model_dict),type(overlap_dict))
        ...
        print_info_message('Pretrained basenet model loaded!!aaaaaaaaaa')
    else:
        print_warning_message('Training from scratch!!')
    return model

if __name__ == "__main__":#モジュールを直接実行したときにこの部分が実行される
